...
function isIsoscelesTriangle(a, b, c) {
  ...
  const cCond = c + b > a;
  if (isSide)
    return (a === b && aCond) || (a === c && bCond) || (b === c && cCond);
  return false;
}
...
function convertToRomanNumerals(num) {
  ...
  while (countedNum > 0) {
    if (countedNum >= 10) {
      result += 'X';
      countedNum -= 10;
    } else if (countedNum === 9) {
      result += 'IX';
      countedNum -= 9;
    } else if (countedNum >= 5) {
      result += 'V';
      countedNum -= 5;
    } else if (countedNum === 4) {
      result += 'IV';
      countedNum -= 4;
    } else {
      result += 'I';
...
function sortByAsc(arr) {
  ...
  const sortedArr = [...sortByAsc(first), elem, ...sortByAsc(second)];
